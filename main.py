...
#___________________________________TEMP_DATA_PUSH_URL-START___________________________________# 
# NOTE: this is for the Data Push URL
@app.route('/v0.5/health-information/transfer', methods=['POST'])
def health_info_receive():
    print("Data Push URL LOG: Data being received!")
    print(request.json)

    # Here match the incoming trxn ID with the one in DB and this will help get all information 
    #   for the consent artefact connected to this trxn ID (like the expiry date, signature etc.)
    # NOTE: if no trxn ID then simply ignore!
    full_req_data = request.json
    ...
    #     }
    # }
    # Store the trxn ID and all pages received PROPERLY (need to check decryption) 
    #  - once all pages properly received then store time at which done in DB for that consent artefact ID
    #    and trigger send f'{GATEWAY}/v0.5/health-information/notify'
    #    - via the trxn ID we can get the consent artefact for this - send notify for that
    #    - this must be sent through the server that is handling APIs other than data push url
    #    - a function for the same is below this route

    # Multiple other steps - like decryption and storing in the DB
    # In the end we need some trigger letting us know that the consent Artefact transfer is done
    # - send f'{GATEWAY}/v0.5/health-information/notify' for each consent artefact that is done
    #    - this must be sent through the server that is handling APIs other than data push url
    #    - this API is in postman

    return jsonify(summary = {"Data Push URL": "Data rcv"})

# notify function triggered once the data transfer finished/failed
def send_hi_notify(con_art_id, trxn_id):
    ntfy_url = f"{GATEWAY_HOST}/v0.5/health-information/notify"
    ...
    payload = json.dumps({
        ...
        "notification": {
            ...
            "doneAt": "<get from DB for the respective con_art_id>",
            "notifier": {
                "type": "HIU",
                "id": "HIUTst1"
            },
    ...
    hi_notify_resp = requests.request("POST", ntfy_url, headers=headers, data=payload)
    print("--------- HIU LOG: HIU has sent Health Info Notify to CM ----------")
    print(hi_notify_resp)
#___________________________________TEMP_DATA_PUSH_URL - END___________________________________#
...
# have some trigger like push button on HIU app to start HI request and get transfer done
@app.route('/v0.5/health-information/hiu/on-request', methods=['POST'])
def hi_on_request():
    print("HIU LOG: HI on request received!")
    print(request.json)

    # For the original hiu/request call remember the request ID in the DB 
    #   for the specific consent artefact too!
    # The response here will give a trxn ID and the response req ID
    # - assign this trxn ID to the specific response req ID in DB
    req_data = request.json

    # get the following information
    req_data = request.json
    req_hi_info = req_data['hiRequest']
    req_hi_trxn_id = req_hi_info['transactionId']
    req_hi_sess_status = req_hi_info['sessionStatus']

    # these trxn IDs will be used by Data Push URL to confirm correct data retrieval
